...
% Check that the right event file is picked up
...
assert(isequal(...
    ...
    'sub-01', ...
    'func', ...
    'sub-01_task-stopsignalwithletternaming_run-01_events.tsv')));
...
% Check that the right event file is picked up when in the root folder.
event_file = bids.query(BIDS, 'data', ...
                        'task', 'nback', ...
                        'type', 'events');

% just to make sure the file exists
assert(isequal(exist(fullfile(pwd, ...
    'bids-examples', ...
    'synthetic', ...
    'task-nback_events.tsv'), 'file'), ...
    2))

% just to make sure that it is picked up by query
assert(isequal(...
    event_file{1}, ...
    fullfile(pwd, 'bids-examples', 'synthetic', 'task-nback_events.tsv')));
