...
function test_parse_filename_warnings()
  ...
  for i = 1:size(filename_error, 1)
    for j = 1:numel(filename_error{i, 1})
      ...
      assertWarning(@()bids.internal.parse_filename(filename_error{i, 1}{j}, fields, ...
                                                    tolerant, true), ...
                    ['parse_filename:' filename_error{i, 2}]);

      p = bids.internal.parse_filename(filename_error{i, 1}{j}, fields, tolerant, false);
...
function test_parse_filename_participants()
  ...
  expected = struct('filename', 'participants.tsv', ...
                    'suffix', 'participants', ...
                    'entities', struct(), ...
                    'ext', '.tsv', ...
                    'prefix', '');
...
function test_parse_filename_wrong_template()
  ...
  assertWarning(@()bids.internal.parse_filename(filename, {'echo'}, false), ...
