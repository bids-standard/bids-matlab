...
elseif nargin == 1
    if ischar(root)
        ...
        error('Invalid input: root must be a char filename; got a %s',...
            class(root));
    end
end
...
if ~exist(out.dir,'dir')
    error('BIDS directory does not exist: ''%s''',out.dir);
elseif ~exist(fullfile(out.dir,'dataset_description.json'),'file')
...
%==========================================================================
try
    out.description = bids.util.jsondecode(fullfile(out.dir,'dataset_description.json'));
catch err
    error('BIDS dataset description could not be read: %s',err.message);
end
if ~isfield(out.description,'BIDSVersion')
    error('BIDS dataset description not valid: missing BIDSVersion field');
end
out.bidsVersion = out.description.BIDSVersion;
if ~isfield(out.description,'Name')
    error('BIDS dataset description not valid: missing Name field');
end
out.name = out.description.Name;
...
%==========================================================================
p = bids.internal.file_utils('FPList',out.dir,'^participants\.tsv$');
if ~isempty(p)
    out.participants = bids.util.tsvread(p);
end
p = bids.internal.file_utils('FPList',out.dir,'^participants\.json$');
if ~isempty(p)
    out.participants.meta = bids.util.jsondecode(p);
end
...
%-Subjects
%==========================================================================
sub = cellstr(bids.internal.file_utils('List',out.dir,'dir','^sub-.*$'));
if isequal(sub,{''})
    error('No subjects found in BIDS directory.');
end

for su=1:numel(sub)
    sess = cellstr(bids.internal.file_utils('List',fullfile(out.dir,sub{su}),'dir','^ses-.*$'));
    for se=1:numel(sess)
        if isempty(out.subjects)
            out.subjects = parse_subject(out.dir, sub{su}, sess{se});
        else
            out.subjects(end+1) = parse_subject(out.dir, sub{su}, sess{se});
        end
    end
end
...
%==========================================================================
    function subject = parse_subject(p, subjname, sesname)
        ...
        if exist(pth,'dir')
            f = bids.internal.file_utils('List',pth,...
            ...
            for i=1:numel(f)
                ...
                %------------------------------------------------------------------
                p = bids.internal.parse_filename(f{i}, {'sub','ses','acq','ce','rec','fa','echo','inv','run'});
        ...
        if exist(pth,'dir')
            ...
            for i=1:numel(f)
                ...
                p = bids.internal.parse_filename(f{i}, {'sub','ses','task','acq','rec','fa','echo','inv','run','recording', 'meta'});
        ...
        if exist(pth,'dir')
            ...
            if any(~cellfun(@isempty,labels))
                ...
                for i=1:numel(idx)
                    fb = bids.internal.file_utils(bids.internal.file_utils(f{idx(i)},'basename'),'basename');
        ...
        if exist(pth,'dir')
            ...
            for i=1:numel(f)
                ...
                p = bids.internal.parse_filename(f{i}, {'sub','ses','task','acq','run','meta'});
                switch p.ext
        ...
        if exist(pth,'dir')
            ...
            %-MEG data file
            %----------------------------------------------------------------------
            [f,d] = bids.internal.file_utils('List',pth,...
                sprintf('^%s.*_task-.*_meg\\..*[^json]$',subject.name));
            if isempty(f), f = d; end
            if isempty(f), f = {}; else f = cellstr(f); end
            for i=1:numel(f)
                
                p = bids.internal.parse_filename(f{i}, {'sub','ses','task','acq','run','proc', 'meta'});
        ...
        if exist(pth,'dir')
            f = bids.internal.file_utils('FPList',pth,...
            ...
            for i=1:numel(f)
                ...
                % recordings
                %------------------------------------------------------------------
                p = bids.internal.parse_filename(f{i}, {'sub','ses','task'});
        ...
        if exist(pth,'dir')
            ...
            for i=1:numel(f)
                ...
                %------------------------------------------------------------------
                p = bids.internal.parse_filename(f{i}, {'sub','ses','acq','run', 'bval','bvec'});
                subject.dwi = [subject.dwi p];
                
                %-bval file
                %------------------------------------------------------------------
                % bval file can also be stored at higher levels (inheritance principle)
                bvalfile = bids.internal.get_metadata(f{i},'^.*%s\\.bval$');
                ...
                bvecfile = bids.internal.get_metadata(f{i},'^.*%s\\.bvec$');
                if isfield(bvalfile,'filename')
        ...
        if exist(pth,'dir')
            ...
            for i=1:numel(f)
                ...
                %-PET imaging file
                %------------------------------------------------------------------
                p = bids.internal.parse_filename(f{i}, {'sub','ses','task','acq','rec','run'});
