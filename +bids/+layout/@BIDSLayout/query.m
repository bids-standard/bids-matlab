function result = query(this,query,varargin)
% QUERY Query a BIDS layout for various data items
%
...
switch query
    case 'modalities'
        hasmod = arrayfun(@(y) structfun(@(x) isstruct(x) & ~isempty(x),y),...
            this.subjects,'UniformOutput',false);
        hasmod = any([hasmod{:}],2);
        mods   = fieldnames(this.subjects)';
        result = mods(hasmod);
    case {'sessions', 'subjects', 'tasks', 'runs', 'types', 'data', 'metadata'}
        ...
        if any(ismember(opts(:,1),'sub'))
            ...
            subs = unique({this.subjects.name});
        ...
        if any(ismember(opts(:,1),'modality'))
            ...
            mods = bids.query(this,'modalities');
        end
        ...
        for i=1:numel(this.subjects)   
            %-Only continue if this subject is one of those filtered
            if ~ismember(this.subjects(i).name(5:end),subs), continue; end
            for j=1:numel(mods)
                d = this.subjects(i).(mods{j});
                for k=1:numel(d)
                    ...
                    switch query
                        case 'subjects'
                            if sts
                                result{end+1} = this.subjects(i).name;
                            end
                        case 'sessions'
                            if sts
                                result{end+1} = this.subjects(i).session;
                            end
                        case 'data'
                            if sts && isfield(d(k),'filename')
                                result{end+1} = fullfile(this.subjects(i).path,mods{j},d(k).filename);
                            end
                        case 'metadata'
                            if sts && isfield(d(k),'filename')
                                f = fullfile(this.subjects(i).path,mods{j},d(k).filename);
