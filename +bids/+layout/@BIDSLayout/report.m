function report(this, Subj, Ses, Run, ReadNII)
% REPORT Create a short summary of the acquisition parameters of a BIDS dataset
%
% bids.report(this, Subj, Ses, Run, ReadNII)
%
% INPUTS:
% - this: A BIDSLayout object
%
...
%-Get sessions
%--------------------------------------------------------------------------
subjs_ls = query(this,'subjects');
sess_ls = query(this,'sessions', 'sub', subjs_ls(Subj));
if isempty(sess_ls)
    sess_ls = {''};
end
if Ses == 0
...
for iSess = Ses
    ...
    types_ls = query(this,'types', 'sub', subjs_ls(Subj), 'ses', sess_ls(iSess));
    tasks_ls = query(this,'tasks', 'sub', subjs_ls(Subj), 'ses', sess_ls(iSess));
    ...
    for iType = 1:numel(types_ls)
        ...
        switch types_ls{iType}
            ...
            case {'T1w' 'inplaneT2' 'T1map' 'FLASH'}
                ...
                % get the parameters
                acq_param = get_acq_param(this, subjs_ls{Subj}, sess_ls{iSess}, ...
            ...
            case 'bold'
                ...
                for iTask = 1:numel(tasks_ls)
                    ...
                    runs_ls = query(this,'runs','sub', subjs_ls{Subj}, 'ses', sess_ls{iSess}, ...
            ...
            case 'dwi'
                ...
                try
                    acq_param.n_vecs = num2str(size(this.subjects(Subj).dwi.bval,2));
                    %             acq_param.bval_str = ???
                catch
...
%==========================================================================
function acq_param = get_acq_param(this, subj, sess, type, task, run, ReadGZ)
...
if ismember(type, {'T1w' 'inplaneT2' 'T1map' 'FLASH' 'dwi'})
    ...
    filename = query(this, 'data', 'sub', subj, 'ses', sess, 'type', type);
    metadata = query(this, 'metadata', 'sub', subj, 'ses', sess, 'type', type);
...
elseif strcmp(type, 'phasediff')
    ...
    filename = query(this, 'data', 'sub', subj, 'ses', sess, 'type', type, 'run', run);
    metadata = query(this, 'metadata', 'sub', subj, 'ses', sess, 'type', type, 'run', run);
